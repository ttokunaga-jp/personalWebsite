...
test.beforeEach(async ({ page }) => {
  ...
  await page.route("**/api/v1/public/research", async (route) => {
    await route.fulfill({ json: researchEntriesFixture });
  });

  await page.route("**/api/v1/public/projects", async (route) => {
    await route.fulfill({ json: projectsFixture });
  });

  await page.route("**/api/v1/public/contact/availability", async (route) => {
    await route.fulfill({ json: contactAvailabilityFixture });
  });

  await page.route("**/api/v1/public/contact/config", async (route) => {
    await route.fulfill({ json: contactConfigFixture });
  });

  await page.route("**/api/v1/public/contact/bookings", async (route, request) => {
