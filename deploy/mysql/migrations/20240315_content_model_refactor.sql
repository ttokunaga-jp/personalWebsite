-- Migration: Content Model Refactor (Phase 1)
-- Creates new content management tables, preserves previous tables by renaming with legacy_ prefix,
-- and initialises schema_migrations tracking.

-- Execute this script against the application database (e.g. `portfolio`).
...
START TRANSACTION;
INSERT IGNORE INTO schema_migrations (name) VALUES ('20240315_content_model_refactor');
COMMIT;

DROP PROCEDURE IF EXISTS rename_if_exists;
DELIMITER $$
CREATE PROCEDURE rename_if_exists(IN old_name VARCHAR(128), IN new_name VARCHAR(128))
BEGIN
...
CALL rename_if_exists('profile', 'legacy_profile');
CALL rename_if_exists('profile_skills', 'legacy_profile_skills');
CALL rename_if_exists('profile_focus_areas', 'legacy_profile_focus_areas');
CALL rename_if_exists('projects', 'legacy_projects');
CALL rename_if_exists('project_tech_stack', 'legacy_project_tech_stack');
CALL rename_if_exists('research', 'legacy_research');
CALL rename_if_exists('blog_posts', 'legacy_blog_posts');
CALL rename_if_exists('blog_post_tags', 'legacy_blog_post_tags');
CALL rename_if_exists('meetings', 'legacy_meetings');
CALL rename_if_exists('contact_messages', 'legacy_contact_messages');

DROP PROCEDURE rename_if_exists;

-- Recreate schema according to docs/content-model-refactor.md
